...
class ProgressTracker:
    ...
    def get_achievements(self):
        """
        Calculate dynamic achievements based on user workout history
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        # 1. First Steps (Complete 5 workouts)
        ...
        achievements.append({
            ...
            "points": 50,
            "earned": total_workouts >= 5,
            "icon": "ri-run-line",
            "progress": min(total_workouts, 5),
            "total": 5
        })

        # 2. Iron Man (Lift/Push total of 500 reps)
        cursor.execute('SELECT SUM(repetitions) FROM workouts')
        ...
        achievements.append({
            ...
            "points": 100,
            "earned": total_reps >= 500,
            "icon": "ri-weight-line",
            "progress": min(total_reps, 500),
            "total": 500
        })

        # 3. Consistency King (Work out on 3 distinct days)
        cursor.execute('SELECT COUNT(DISTINCT DATE(timestamp)) FROM workouts')
        distinct_days = cursor.fetchone()[0] or 0
        achievements.append({
            "id": "consistency",
            "title": "Consistency King",
            "description": "Work out on 3 distinct days",
            "points": 150,
            "earned": distinct_days >= 3,
            "icon": "ri-calendar-check-line",
            "progress": min(distinct_days, 3),
            "total": 3
        })
    ...
    def get_exercise_improvements(self):
        """
        Calculates simple percentage improvement metrics per exercise.
        Compares average reps of last 2 sessions vs all prior sessions.
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        
        cursor.execute('SELECT DISTINCT exercise_type FROM workouts')
        exercises = [row[0] for row in cursor.fetchall()]
        
        improvements = []
        for ex in exercises:
            cursor.execute('''
                SELECT repetitions FROM workouts 
                WHERE exercise_type = ? 
                ORDER BY timestamp DESC LIMIT 2
            ''', (ex,))
            recent_reps = [r[0] for r in cursor.fetchall()]
            
            if len(recent_reps) > 0:
                avg_recent = sum(recent_reps) / len(recent_reps)
                improvements.append({
                    "exercise": ex,
                    "improvement": f"avg {round(avg_recent)} reps" # Simple absolute display for UI
                })
    ...
    def get_total_errors(self):
        """Get total number of corrections/errors logged"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        
        # We classify form corrections or warnings as errors
        cursor.execute("SELECT COUNT(*) FROM feedback_history WHERE feedback_type != 'positive'")
        result = cursor.fetchone()
